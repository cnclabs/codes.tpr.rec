...
int main(int argc, char **argv){
    ...
    int num_item = arg_parser.get_int("-num_item", 5, "number of sampled items");
    int num_word = arg_parser.get_int("-num_word", 5, "number of sampled words");
    ...
    for (int w=0; w<worker; w++)
    {
        ...
        while (update < worker_update_times)
        {
            ...
            iw_sampler.feed_sampled_contexts(item_pos, num_word, item2words_pos); // item-words
            item_embed_pos = i_mapper.textgcn_embedding(item2words_pos);
            // [user, word, word, ...]
            item2words_neg.push_back(item_neg);
            iw_sampler.feed_sampled_contexts(item_neg, num_word, item2words_neg); // item-words
            item_embed_neg = i_mapper.textgcn_embedding(item2words_neg);

            if (optimizer.feed_margin_bpr_loss(user_embed, item_embed_pos, item_embed_neg, 8.0, dimension, user_loss, item_loss_pos, item_loss_neg))
            {
                i_mapper.update_with_l2(user, user_loss, alpha, l2_reg);
                i_mapper.update_with_l2(item_pos, item_loss_pos, alpha, l2_reg);
                i_mapper.update_with_l2(item_neg, item_loss_neg,alpha, l2_reg);
                for (auto it=++user2items.begin(); it!=user2items.end(); it++)
                    i_mapper.update_with_l2(*it, user_loss, alpha/num_item, l2_reg);
                for (auto it=++item2words_pos.begin(); it!=item2words_pos.end(); it++)
                    i_mapper.update_with_l2(*it, item_loss_pos, alpha/num_word, l2_reg);
                for (auto it=++item2words_neg.begin(); it!=item2words_neg.end(); it++)
                    i_mapper.update_with_l2(*it, item_loss_neg, alpha/num_word, l2_reg);
    ...
    monitor.end();
    i_mapper.save_gcn_to_file(&ui_file_graph, ui_file_graph.get_all_from_nodes(), save_name, 0);
    i_mapper.save_gcn_to_file(&iw_file_graph, ui_file_graph.get_all_to_nodes(), save_name, 1);
    i_mapper.save_gcn_to_file(&iw_file_graph, iw_file_graph.get_all_to_nodes(), save_name, 1);
