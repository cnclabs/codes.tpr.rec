...
int main(int argc, char **argv){
    ...
    std::string save_name = arg_parser.get_str("-save", "walklets.embed", "path for saving mapper");
    ...
    int window_min = arg_parser.get_int("-window_min", 1, "min window size of skipgram");
    int window_max = arg_parser.get_int("-window_max", 5, "max window size of skipgram");
    ...
    for (int t=0; t<walk_times; t++)
    {
        ...
        for (long index=0; index<sampler.vertex_size; index++)
        {
            ...
            std::vector<std::vector<long>> walk = sampler.draw_scaledskipgram(index, walk_length-1, window_min, window_max);
