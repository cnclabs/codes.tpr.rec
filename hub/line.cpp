...
int main(int argc, char **argv){
    ...
    std::string save_name = arg_parser.get_str("-save", "line.embed", "path for saving mapper");
    ...
    int order = arg_parser.get_int("-order", 1, "1 for 1st-order modeling; 2 for 2nd-order modeling");
    ...
    long dimension2=0;
    if (order==2) dimension2 = dimension;
    LookupMapper c_mapper(sampler.vertex_size, dimension2); // as context
    ...
    for (int w=0; w<worker; w++)
    {
        ...
        if (order==1)
        {
            while (update < worker_update_times)
            {
                ...
                context = sampler.draw_a_context(vertex);
                optimizer.feed_loglikelihood_loss(v_mapper[vertex], v_mapper[context], 1.0, dimension, v_loss, c_loss);
                v_mapper.update(context, c_loss, alpha);
                ...
                for (int n=0; n<num_negative; n++)
                {
                    ...
                    optimizer.feed_loglikelihood_loss(v_mapper[vertex], v_mapper[context], 0.0, dimension, v_loss, c_loss);
                    v_mapper.update(context, c_loss, alpha);
                    c_loss.assign(dimension, 0.0);
                }
                v_mapper.update(vertex, v_loss, alpha);
        ...
        else if (order==2)
        {
            while (update < worker_update_times)
            {
                ...
                c_mapper.update(context, c_loss, alpha);
                c_loss.assign(dimension, 0.0);
