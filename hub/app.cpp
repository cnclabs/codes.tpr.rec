...
int main(int argc, char **argv){
    ...
    std::string save_name = arg_parser.get_str("-save", "app.embed", "path for saving mapper");
    ...
    double jump_prob = arg_parser.get_double("-jump_prob", 0.15, "probability of ending a walk");
    ...
    for (int t=0; t<walk_times; t++)
    {
        ...
        {
            long vertex=index, context;
            std::vector<double> vertex_loss(dimension, 0.0);
            std::vector<double> context_loss(dimension, 0.0);
            std::vector<long> walk = sampler.draw_a_jump_walk(index, jump_prob);

            for (auto context: walk)
            {
                // positive
